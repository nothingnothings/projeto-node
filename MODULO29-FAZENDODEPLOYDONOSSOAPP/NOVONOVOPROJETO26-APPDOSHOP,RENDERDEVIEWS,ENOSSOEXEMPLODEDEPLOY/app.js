...
const https = require('https'); /////ATÉ AGORA, USAMOS HTTP... -_> MAS NESTE MÓDULO DO DEPLOY, VAMOS QUERER USAR O HTTPS (que é http com ENCRYPTION SSL).... --> FAREMOS ISSO nesse nosso arquivo starter 'app.js'.... 

///para conseguir USAR O PROTOCOLO 'https', PRECISAMOS TER OS ARQUIVOS DE 'server.key'(A PRIVATE KEY, MANTIDA NO NOSSO SERVER) e 'server.cert' ( o SSL CERTIFICATE, que contém nossa PUBLIC KEY, QUE SERÁ ENVIADA A CADA UM DE NOSSOS USER....) -------> PARA CRIAR ESSES 2 ARQUIVOS NO SEU PROJETO, você precisa executar o código, no terminal, de ''' openssl req -nodes -new -x509 -keyout server.key -out server.cert '''' 
...
const helmet = require('helmet'); ///usado para INCREMENTAR A SEGURANÇA DAS NOSSAS RESPONSES, POR MEIO DO SET DE HEADERS ADICIONAIS NELAS, POR MEIO DE SEU MIDDLEWARE..
...
// const MONGODB_URI = 'mongodb+srv://madblorga:T5lws5TGxtclEbKI@cluster0.nhtjo.mongodb.net/shop?retryWrites=true&w=majority'; ///VERSÃO _ SEM USO DE ENVIRONMENT VARIABLES NODE....
...
////obs: o heroku NÃO PRECISA DESSAS CONSTANTES, NEM DO 'privatekey', nem do 'certificate' (que é nossa PUBLIC KEY, na verdade.... é a key entregue aos user)... -> o heroku faz isso AUTOMATICAMENTE PARA NÓS... (gera essas coisas automaticamente para nós)....
...
///obs: o HEROKU __ NÃO TE OFERECE COMPRESSION OUT OF THE BOX, por isso você precisa manter esse middleware, quando hosteia seu app/backend nele...
...
// app.use(csrfProtection); ///inicializa nossa PROTEÇÃO CONTRA ATAQUES CSRF.... sempre coloque __DEPOIS__ de 'multer'... --> colocamos, eventualmente, DEPOIS DO RUN DA ROUTE DE 'stripe', para que O REQUEST ENVIADO AO STRIPE (form do stripe) NÃO SEJA 'VERIFIED for the presence of a csrfToken'....
...
app.use((req, res, next) => {
  ...
  // res.locals.csrfToken = req.csrfToken(); ///agora vamos settar isso EM UM MIDDLEWARE LÁÁÁ´´A EMBAIXO, isso pq escrevemos 'app.use(csrfProtection)' lá embaixo também...
...
app.post('/create-order', authCheckerAndRedirecter, shopController.orderPost); ////transplantado lá do arquivo 'route' de 'shop.js'...
///transplantamos essa route aqui, em 'app.js', PQ NÃO QUEREMOS QUE O csurf VERIFIQUE A PRESENÇA DE CSRFTOKEN dentro de requests enviados a essa route...

app.use(csrfProtection); ///inicializa nossa PROTEÇÃO CONTRA ATAQUES CSRF.... sempre coloque __DEPOIS__ de 'multer'...

app.use((req, res, next) => { ///COLOCAMOS AQUI.... EIS O CÓDIGO EM QUESTÃO. precisamos settar isso depois do middleware de 'csrfProtection'...
...
mongoose
  ...
  .then((result) => {
    ...
    // app.listen(3000);  ///VERSÃO SEM USO DE ENVIRONMENT VARIABLES...


  ///VERSÃO __ HTTP__ de nosso código (e não 'HTTPS', que requer o CERTIFICATE/PUBLIC KEYE A PRIVATE KEY..)
  //  app.listen(process.env.PORT || 3000);  //  ---> OK .... MAS VAMOS COLOCAR UMA GAMBIARRA,   (a grande maioria dos hosting providers vai enfiar uma variável de 'PORT' no seu runtime AUTOMATICAMENTE, o que fará com que esse valor seja preenchido...)
    ...
    app.listen(process.env.PORT || 3000); ///REVERTEMOS NOSSO CÓDIGO à VERSÃO 'http', JUSTAMENTE PQ VAMOS QUERER 'DEIXAR O NOSSO HOSTING PROVIDER FAZER O HANDLE DE SSL' (a parte 's' em 'https'...)

    // https.createServer( ///geralmente os HOSTING PROVIDERS VÃO FAZER O MANAGE DO SSL _ PARA _ VOCÊ (aí você não precisa fazer nada), MAS É LÓGICO QUE É BOM __ SABER_ _ COMO 'ENABLAR' essa ENCRYPTION SSL por conta própria... (apesar de isso não funcionar/ser reconhecido como SUSPEITO PELO CHROME.... ------> 'self-signed certificates' SÃO __ SUSPEITOS/TIDOS COMO SUSPEITOS PELO BROWSER....)
    //   {
    //     key: privateKey,
    //     cert: certificate
    //   },
